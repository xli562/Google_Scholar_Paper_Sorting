...
def is_constantinides_among_first_three(authors_div):
    ...
    return any('GA Constantinides' in author for author in authors_list[:4])
...
for row in rows:
    ...
    if year_span and int(year_span.text if year_span.text != '' else 0) >= 2018:
        title_a = row.find('a', class_='gsc_a_at')
        if title_a and '' in title_a.text:
            ...
            if is_constantinides_among_first_three(authors_div):
                ...
                citations = int(citations_a.text) if citations_a.text != '' else 0
...
df.to_excel('filtered_articles_newer.xlsx', index=False, engine='openpyxl')
